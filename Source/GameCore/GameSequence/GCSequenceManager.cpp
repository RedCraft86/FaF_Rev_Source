...
void UGCSequenceManager::SetGameSequence(const FGCSequenceID InSequenceID)
{
	...
	if (const UGCSaveManager* SaveManager = ThisSequenceID.IsValid() ? UGCSaveManager::Get(this) : nullptr)
	{
...
void UGCSequenceManager::OnWorldLoaded()
{
	...
	if (Data.IsValidData())
	{
		...
		for (const TPair<FString, float>& CustomNumber : Data.CustomNumberData)
		{
