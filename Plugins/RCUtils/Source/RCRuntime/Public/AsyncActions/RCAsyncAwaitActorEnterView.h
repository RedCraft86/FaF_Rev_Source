...
#include "RCAsyncAwaitActorEnterView.generated.h"

UCLASS()
class RCRUNTIME_API URCAsyncAwaitActorEnterView final : public URCAsyncActionBase
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(BlueprintAssignable)
		FRCAsyncActionDelegate EnteredScreen;

	UPROPERTY(BlueprintAssignable)
		FRCAsyncActionDelegate NeverEnteredScreen;

	/* Waits the specified amount of time and checks if the specified actor ever enters player view.
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "AsyncActions", meta = (BlueprintInternalUseOnly = "true", DefaultToSelf = "TargetActor", DisplayName = "Await Actor Enter Player View", Keywords = "actor enter player screen view"))
		static URCAsyncAwaitActorEnterView* AwaitActorEnterView(const AActor* TargetActor, float MaxDistance = 5000.0f, bool bOriginOnly = false, bool bLineTraceCheck = true, FActorBoundsCheckParams TraceCheckParams = FActorBoundsCheckParams(), float WaitTime = 15.0f, float CheckInterval = 0.15f);
