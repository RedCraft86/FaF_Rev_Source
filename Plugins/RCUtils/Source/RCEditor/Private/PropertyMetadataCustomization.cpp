...
#define LOCTEXT_NAMESPACE "RCEditor"
...
TSharedPtr<IDetailCustomization> FPropertyMetadataCustomization::MakeInstance(TSharedPtr<IBlueprintEditor> InBlueprintEditor)
{
	...
	if (Objects && Objects->Num() == 1)
	{
		...
		{
			return MakeShareable(new FPropertyMetadataCustomization(InBlueprintEditor, Blueprint));
		}
	}
...
TArray<FBPVariableMetaDataEntry> FPropertyMetadataCustomization::GetMetadataEntries(int PropertyIndex) const
{
	if (!BlueprintPtr->NewVariables.IsValidIndex(PropertyIndex))
		return TArray<FBPVariableMetaDataEntry>();

	TArray<FBPVariableMetaDataEntry> A;
	TArray<FBPVariableMetaDataEntry> B;

	TArray<FBPVariableMetaDataEntry> Unsorted = BlueprintPtr->NewVariables[PropertyIndex].MetaDataArray;
...
{
	if (!Property.IsValid()) // Hopefully this never nulls since the editor will crash lmao
		return nullptr;
...
int FPropertyMetadataCustomization::GetMetadataCount(FName Property) const
{
	if (Property.IsNone())
		return 0;
	...
	if (VarIndex != INDEX_NONE)
		return BlueprintPtr->NewVariables[VarIndex].MetaDataArray.Num();

	return 0;
}

bool FPropertyMetadataCustomization::DoesMetadataExist(FName Property, FName Key) const
{
	if (Property.IsNone() || Key.IsNone())
		return false;
...
void FPropertyMetadataCustomization::RemoveMetadata(FName Property, FName Key) const
{
	if (!DoesMetadataExist(Property, Key))
		return;

	FBlueprintEditorUtils::RemoveBlueprintVariableMetaData(BlueprintPtr.Get(), Property, nullptr, Key);
}

FString FPropertyMetadataCustomization::GetMetadataValue(FName Property, FName Key) const
{
	if (!DoesMetadataExist(Property, Key))
		return FString();
...
void FPropertyMetadataCustomization::SetMetadataValue(FName Property, FName Key, const FString& Value) const
{
	if (Property.IsNone() || Key.IsNone())
		return;
...
void FPropertyMetadataCustomization::ChangeMetadataKey(FName Property, FName OldKey, FName NewKey) const
{
	if (Property.IsNone() || OldKey.IsNone())
		return;
	...
	else
	{
		...
		if (Value.IsEmpty())
			Value = GetMetadataValue(Property, OldKey);
...
void FPropertyMetadataCustomization::ChangeMetadataValue(FName Property, FName Key, const FString& Value, bool bDeleteEmpty) const
{
	if (Property.IsNone() || Key.IsNone())
		return;
...
void FPropertyMetadataCustomization::VerifyMetadata(FName Property, FName Key) const
{
	if (Property.IsNone() || Key.IsNone())
		return;
